...
__global__ void setBondKernel(interactionBonded solidBond2Solid, 
    ...
    solid s)
{
	...
	solidBond2Solid.objectPointed[idx] = idx_i;
	solidBond2Solid.objectPointing[idx] = idx_j;
    solidBond2Solid.contactNormal[idx] = make_double3(0, 0, 0);
    double3 pos_i = s.points.position[idx_i];
    double3 pos_j = s.points.position[idx_j];
    solidBond2Solid.contactPoint[idx] = 0.5 * (pos_i + pos_j);
    ...
    if(s.clusterID[idx_i] >= 0 && s.clusterID[idx_i] == s.clusterID[idx_j])
	{
		int mat_i = s.materialID[idx_i];
		int mat_j = s.materialID[idx_j];
		int c_ij = contactModels.getCombinedIndex(mat_i, mat_j);
        if (contactModels.bonded.E[c_ij] > 1.e-10)
        {
            solidBond2Solid.isBonded[idx] = 1;
            double rad_i = s.radius[idx_i];
            double rad_j = s.radius[idx_j];
            double normalOverlap = rad_i + rad_j - length(pos_i - pos_j);
            double3 contactNormal = normalize(pos_i - pos_j);
            double3 contactPoint = pos_j + (rad_j - 0.5 * normalOverlap) * contactNormal;
            solidBond2Solid.contactNormal[idx] = contactNormal;
            solidBond2Solid.contactPoint[idx] = contactPoint;
        }
	}
}
...
{
	neighborSearch(d, maxThreadsPerBlock);
	d.solidBond2Solid.release();
	d.solidBond2Solid.alloc(d.solid2Solid.num);
	int grid = 1, block = 1;
	int numObjects = d.solid2Solid.num;
	computeGPUParameter(grid, block, numObjects, maxThreadsPerBlock);
	setBondKernel << <grid, block >> > (d.solidBond2Solid,
		d.solid2Solid,
		d.contactModels,
		d.solids);
}